...
- После применения шаблона пользователю уходит локализованное уведомление об окончании подписки.
...
2. Задайте в `.env` имя шаблона — например `EXPIRED_TEMPLATE_NAME=expired`.
3. Убедитесь, что в панели включена отправка webhook-уведомлений и что ручка `PASARGUARD_NOTIFY_PATH` доступна боту.

Если `EXPIRED_TEMPLATE_NAME` не задано, применение шаблона выключено, а событие `user_expired` просто игнорируется.
